...
def loopallframes2vid(num_frames, date=time.localtime(), downsampling_factor=1, destination='.', framerate=125, num_cams=4):
...
if __name__ == '__main__':
     if len(list(sys.argv)) == 2:
          loopallframes2vid(num_frames=sys.argv[1])
     elif len(list(sys.argv)) == 3:
          loopallframes2vid(num_frames=sys.argv[1], date=sys.argv[2], downsampling_factor=sys.argv[3])
     elif len(list(sys.argv)) == 4:
          loopallframes2vid(num_frames=sys.argv[1], date=sys.argv[2], downsampling_factor=sys.argv[3], destination=sys.argv[4])
     elif len(list(sys.argv)) == 5:
          loopallframes2vid(num_frames=sys.argv[1], date=sys.argv[2], downsampling_factor=sys.argv[3], destination=sys.argv[4], framerate=sys.argv[5])
     elif len(list(sys.argv)) == 6:
          loopallframes2vid(num_frames=sys.argv[1], date=sys.argv[2], downsampling_factor=sys.argv[3], destination=sys.argv[4], framerate=sys.argv[5], num_cams=sys.argv[6])
     else:
