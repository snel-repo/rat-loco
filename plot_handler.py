...
def sort_plot(
    ...
    if sort_method == "thresholding":
        ...
        sort_method == "none"
    ):  # do not include bottom row for sorted spikes, nor plot colored dots
        number_of_channels = len(
            np.where(
                (np.array(ephys_channel_idxs_list) != 16)
                & (np.array(ephys_channel_idxs_list) != -1)
    ...
    if len(bodyparts_list) > 0:
        ...
        if (
            bodypart_ref_filter
            and origin_offsets is not False
    ...
    for iChannel, channel_number in enumerate(ephys_channel_idxs_list):
        ...
        if sort_method == "none":
            sorted_spikes_row_space = 0
        else:
            sorted_spikes_row_space = 1
            for iUnit, iUnitKey in enumerate(UnitKeys):
                if channel_number not in [-1, 16]:
                    ...
                    if sort_method == "thresholding" or (
                        sort_method == "kilosort" and iChannel == 0
                    ):
...
def bin_and_count_plot(
    ...
    if sort_method == "kilosort":
        order_by_count = np.arange(len(MU_spikes_count_across_all_steps))
        MU_iter = plot_units
        MU_spikes_count_across_all_steps[::-1].sort()
    else:
        order_by_count = np.argsort(MU_spikes_count_across_all_steps)[::-1]
        MU_iter = MU_spikes_dict.keys()

    # for iUnit, iUnitKey in enumerate(np.fromiter(MU_spikes_dict.keys(),'int')[order_by_count]):
    # import pdb; pdb.set_trace()
    for iUnit, iUnitKey in enumerate(np.fromiter(MU_iter, "int")[order_by_count]):
        try:
...
def raster_plot(
    ...
    # check if any spikes are missing, and if so, exlude them from the raster plot
    # num_spikes_per_unit = np.sum(MU_spikes_3d_array_ephys_time, axis=(0, 1))
    # num_spikes_per_unit = num_spikes_per_unit[num_spikes_per_unit > 0]

    # for each channel and each trial's spike time series, plot onto the raster: plotly scatters
    if sort_method == "kilosort":
        plot_units = np.fromiter(plot_units, "int")
        MU_iter = plot_units
        # MU_iter = plot_units[np.arange(len(num_spikes_per_unit))]
    else:
        MU_iter = MU_step_aligned_spike_idxs_dict.keys()
        # MU_iter = np.fromiter(MU_iter, "int")[np.arange(len(num_spikes_per_unit))]

    for iUnit, iUnitKey in enumerate(MU_iter):
