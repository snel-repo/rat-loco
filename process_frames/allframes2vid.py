...
def allframes2vid(num_frames, downsampling_factor=1, destination=".", framerate=125, num_cams=4):
    ...
    with open("unique_base_filenames.txt", "r") as unique_base_filenames:
        ...
        for iFilename in unique_base_filenames_list:
            frames2vid.frames2vid(
                iFilename, num_frames, downsampling_factor, destination, framerate, num_cams
            )
...
if __name__ == "__main__":
    if len(list(sys.argv)) == 2:
        ...
        allframes2vid(
            num_frames=sys.argv[1], downsampling_factor=sys.argv[2], destination=sys.argv[3]
        )
    elif len(list(sys.argv)) == 5:
        allframes2vid(
            num_frames=sys.argv[1],
            downsampling_factor=sys.argv[2],
