...
def allsessionframes2vid(
    num_frames, date="today", downsampling_factor=1, destination=".", framerate=125, num_cams=4
):
    if date == "today":
        dstr = time.localtime()
    ...
    for iDir in date_match_dir_list:
        ...
        allframes2vid.allframes2vid(
            num_frames, downsampling_factor, destination, framerate, num_cams
        )
    try:
...
if __name__ == "__main__":
    if len(list(sys.argv)) == 2:
        ...
        allsessionframes2vid(
            num_frames=sys.argv[1], date=sys.argv[2], downsampling_factor=sys.argv[3]
        )
