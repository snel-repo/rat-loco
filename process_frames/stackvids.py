...
def stackvids(base_filename, stack_type='hstack', destination='.', num_cams=4):
     if stack_type in ['hstack','vstack']:
          ...
          raise Exception("Bad combination of num_cams argument and stack_type argument. Should be 'hstack' or 'vstack' with numcams less than 4.")
...
if __name__ == '__main__':
     ...
     if len(list(sys.argv)) == 2:
          ...
          stackvids(sys.argv[1], stack_type=sys.argv[2], destination=sys.argv[3], num_cams=sys.argv[4])
     else:
          raise Exception("enter at least 1 arguments and no more than 4! :) \nInputs: base_filename, stack_type, destination, num_cams")