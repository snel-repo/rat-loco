...
bodyparts_list = ['palm_L_y','palm_R_y','tailbase_y']#['mtar_L_y','mtar_L_z','mtar_R_y','mtar_R_z']#['palm_L_y', 'palm_L_z','palm_R_y', 'palm_R_z']#['mtar_L_y','mtar_L_z','mtar_R_y','mtar_R_z'] #['palm_L_y']
...
bodypart_ref_filter =  2 # False/int Example: (False to disable filtering of bodypart_for_reference, 10 for 10Hz cutoff lowpass)
trial_reject_bounds_mm = dict(peak=[-10,20],trough=[-20,20]) #mm, False/Integer/Dict, rejects trials outside bounds of the trial average at each bodypart's alignment timepoint. Examples: False / 40 / dict(peak=[10,40],trough=[-10,25] )
...
if plot_type == "sort":
    ...
    process_steps.behavioral_space(
        ...
        trial_reject_bounds_mm, origin_offsets, bodyparts_list, filter_tracking, session_date, rat_name,
...
# multi_smooth performs smoothing of binned spikes, plots results for all chosen conditions
    ...
    big_fig = make_subplots(cols=num_channels,rows=1,shared_xaxes=True,shared_yaxes=True,
                            horizontal_spacing=0.1, vertical_spacing=0.1,
                            subplot_titles=tuple(num_channels*['tmp_title']))
    for iChannel in range(num_channels):
        _, figs = process_spikes.state_space(
            ephys_data_dict, ephys_channel_idxs_list, MU_spike_amplitudes_list,
            ...
            session_date, rat_name, treadmill_speed, treadmill_incline,
            camera_fps, align_to, vid_length, time_frame,
            do_plot=False, plot_units=plot_units, phase_align=phase_align, plot_template=plot_template, MU_colors=MU_colors, CH_colors=CH_colors
            )
        for iPlot in range(len(figs[0].data)):
            big_fig.add_trace(figs[0].data[iPlot], col=iChannel+1,
                row=1)
        # keep track of session recording parameters, and set those for subplot titles
        big_fig.layout.annotations[iChannel].update(text=figs[0].layout.title.text.split('<br>')[1])
        # big_fig.layout.annotations[2*iChannel+1].update(text=figs[0].layout.annotations[1].text)
        # set y-axis titles to those received from bin_spikes()
        big_fig.update_yaxes(title_text=figs[0].layout.yaxis.title.text,
                                col = iChannel+1, row = 1)
        # big_fig.update_yaxes(title_text=figs[0].layout.yaxis2.title.text,
        #                         col = iChannel+1, row = 2)
    # set x-axis titles to those received from bin_spikes()
    big_fig.update_xaxes(title_text=figs[0].layout.xaxis.title.text,col = num_channels, row = 1)
    # big_fig.update_xaxes(title_text=figs[0].layout.xaxis2.title.text,col = iChannel+1, row = 2)
    big_fig.update_xaxes(matches='x')
    # Reduce opacity to see all traces
    # big_fig.update_traces(opacity=0.75)
    # set all titles using received title from bin_spikes()
